...
/* Actual Milktree logo — ghost/outline style for footer */
const FooterLogo = () => (
  <svg
  ...
  >
    <path d="M22.425 48.6883H12.1741V11.8231H22.1974V22.4453H22.9513C24.9853 15.4865 29.2809 11.8561 34.9352 11.8561C41.2648 11.8561 44.3563 15.8648 44.8092 22.595H45.639C47.5996 15.5602 51.6676 11.8561 57.547 11.8561C64.5572 11.8561 68.099 16.6214 68.099 24.4102V48.6883H57.7747V27.2841C57.7747 23.6537 56.1176 21.1581 52.3482 21.1581C47.9766 21.1581 45.2646 24.2579 45.2646 28.721V48.6883H35.0136V27.2841C35.0136 23.6537 33.2807 21.1581 29.5871 21.1581C25.2155 21.1581 22.4276 24.2579 22.4276 28.721V48.6883H22.425Z" fill="#fff"/>
    <path d="M81.1381 13.854H70.8871V48.6882H81.1381V13.854Z" fill="#fff"/>
    <path d="M81.1381 0.2052H70.8871V10.4922H81.1381V0.2052Z" fill="#fff"/>
    <path d="M94.187 48.6884H83.936V0.132202H94.187V48.6884Z" fill="#fff"/>
    <path d="M107.228 26.8297H110.016L120.497 11.8232H130.817V13.6714L118.079 30.3103L131.571 47.8582V48.6909H119.888L109.864 34.3216H107.226V48.6909H96.9749V0.132202H107.226V26.8297H107.228Z" fill="#fff"/>
    <path d="M133.169 19.9471V11.7799V0.0128174H143.42V11.7774H152.842V19.9446H143.42V34.6922C143.42 37.566 144.475 39.003 147.564 39.003C149.148 39.003 150.881 38.5485 152.237 38.0941L153.368 47.3199C150.653 48.1526 148.847 48.6807 144.926 48.6807C136.635 48.6807 133.169 43.1589 133.169 36.3525V19.9395V19.9471Z" fill="#fff"/>
    <path d="M155.764 48.6883V11.8231H165.486V24.4864H166.316C168.578 14.504 171.591 12.4603 176.263 12.4603H177.318V24.2579H175.208C168.049 24.2579 166.015 26.3752 166.015 32.7297V48.6883H155.764Z" fill="#fff"/>
...
const FooterCTA: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const initCal = () => {
      ...
      // Script already loaded from a previous page — just re-bind the inline embed
      if (win.Cal?.loaded) {
        // Small delay to let React render the target div first
        requestAnimationFrame(() => {
          const el = document.getElementById(CAL_ELEMENT_ID);
          if (el && !el.hasChildNodes()) {
            win.Cal.ns[CAL_NAMESPACE]('inline', {
              elementOrSelector: `#${CAL_ELEMENT_ID}`,
              config: { layout: 'month_view', useSlotsViewOnSmallScreen: 'true' },
              calLink: `milktree-agency/${CAL_NAMESPACE}`,
            });
            win.Cal.ns[CAL_NAMESPACE]('ui', { hideEventTypeDetails: false, layout: 'month_view' });
          }
        });
        return;
      }

      // First load — inject the Cal.com embed script
      ...
      <div className="finalcta__container">
        ...
        <Reveal delay={0.32}>
          <div className="finalcta__cal-wrap">
            <div id={CAL_ELEMENT_ID} className="finalcta__cal" />
          </div>
        </Reveal>
